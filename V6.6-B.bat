...
set /p choice=
if not '%choice%'=='' set choice=%choice:~0,1%
if '%choice%'=='1' goto BCDTweaksChoice
if '%choice%'=='2' goto OptimizePerformancesWindows11-10

:BCDTweaksChoice
color b
cls
echo %z%Do you want to use BCD Tweaks ? (Little note, they can cause problems) %q%
echo.
echo %i%Yes = 1%q%
echo.
echo %i%No = 2%q%
...
bcdedit /export "%USERPROFILE%\Desktop\bcdcurrentsettings.txt"
bcdedit /export "%USERPROFILE%\Desktop\bcdbackup.bcd"   
timeout /t 1 /nobreak > NUL
echo Applying MSI via BCD Tweaks
echo (basic value = Deleted)
...
color b
cls
echo %z%Enable or Disable Tasks ?%q%
echo.
echo %i%Enabled (Default) = 1%q%
echo.
echo %i%Disabled = 2%q%
echo.
echo %i%Skip = 3%q%
...
set /p choice=
if not '%choice%'=='' set choice=%choice:~0,1%
if '%choice%'=='1' goto EnableWindowsAndAppsTasks
if '%choice%'=='2' goto DisableWindowsAndAppsTasks
if '%choice%'=='3' goto OptimizePerformancesWindows11-10-2

:EnableWindowsAndAppsTasks
color b
cls
...
echo Done!
timeout /t 1 /nobreak > NUL
goto OptimizePerformancesWindows11-10-2

:DisableWindowsAndAppsTasks
color b
cls
...
:OptimizePerformancesWindows11-10-2
cls
...
goto WindowsSegmentHeapManager

:WindowsSegmentHeapManager
color b
cls
...
echo.
echo %i%skip (Default) = 3%q%
echo.
echo %i%Per-Applications Segment Heap Manager (Could be useful) = 4%q%
echo.
echo %i%Disabling Windows Segment Heap for Certain Apps (Incompatible ones) = 5%q%
color b
set choice=
set /p choice=
if not '%choice%'=='' set choice=%choice:~0,1%
...
if '%choice%'=='3' goto OptimizePerformancesWindows11-10-3
if '%choice%'=='4' goto PerAppsWindowsSegmentHeap
if '%choice%'=='5' goto IncmptblPpsWdnSgmntHpStstd

:IncmptblPpsWdnSgmntHpStstd
color b
cls
echo Disabling Segment Heap for SteamVR
...
timeout /t 1 /nobreak > NUL
pause
echo Segment Heap can "Break" VR Games (Example, Boneworks. If you use a Quest/Meta (So Link) and then SteamVR, well, SteamVR Will NOT Start at all. and so will the game.
echo So we need to disable DashboardLaunch (In-VR, which means you will only have SteamVR Link started)
echo Here is a small bypass, go into SteamVR Directory and place a shortcut of "vrmonitor.exe" on your desktop, start Meta Quest Link and then VrMonitor (WITHOUT STEAM)
echo When you will start a game (ex : VRChat) Steam will start after SteamVR, this is to make sure it doesn't crash (yes it does for some reasons, maybe driver error)
pause

goto OculusDashINVRDisabler

:OculusDashINVRDisabler
color b
cls
echo %z%Do you want to disable Meta Quest Link IN-VR Dashboard ? (Example, to use Only SteamVR)%q%
echo.
echo %i%Yes = 1%q%
echo.
echo %i%Revert = 2%q%
...
set /p choice=
if not '%choice%'=='' set choice=%choice:~0,1%
if '%choice%'=='1' goto MetaQuestLinkDisabler
if '%choice%'=='2' goto MetaQuestLinkEnabler

:MetaQuestLinkDisabler
echo Disabling Meta Quest Link Dashboard. (Default Value = Deleted/0, new value "1")
REG ADD "HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Oculus VR, LLC\Oculus\Config" /v "PreventDashLaunch" /t REG_DWORD /d "1" /f
echo Note! : SteamVR Link Forces the uses of Async, meaning the "Sync FPS" are 45/60/80/100/120/144. if everything here is too much, just Don't use Segment heap at all.
pause
goto OptimizePerformancesWindows11-10-3

:MetaQuestLinkEnabler
echo Enabling Meta Quest Link Dashboard (Default value)
REG ADD "HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Oculus VR, LLC\Oculus\Config" /v "PreventDashLaunch" /t REG_DWORD /d "0" /f
pause
goto OptimizePerformancesWindows11-10-3
...
:OptimizePerformancesWindows11-10-3
cls
...
PowerShell Invoke-WebRequest "https://github.com/ZaneLS/MSI_UTIL_V3/raw/refs/heads/main/selective-acks.txt" -OutFile "%temp%\selective-acks.txt"
start "" /wait "%temp%\selective-acks.txt"
...
echo.
echo %i%Steam GPU Web Acceleration Management = 3%q%
echo.
echo %i%skip = 4%q%
color b
set choice=
...
if '%choice%'=='3' goto SteamGPUAccelDisabler
if '%choice%'=='4' goto InSpectreThings
...
echo Download Process Lasso
PowerShell Invoke-WebRequest "https://dl.bitsum.com/files/processlassosetup64.exe" -OutFile "%temp%\processlassosetup64.exe"
start "" /wait "%temp%\processlassosetup64.exe"
timeout /t 1 /nobreak > NUL
...
if '%choice%'=='2' goto SteamGPUAccelDisabler
color b
...
:SteamGPUAccelDisabler
cls
color b
cls
echo %z%Would you like to Enable or disable GPU Acceleration for Steam ?%q%
echo.
echo %i%Enable (default) = 1%q%
echo.
echo %i%Disable = 2%q%
echo.
echo %i%Skip = 3%q%
...
set /p choice=
if not '%choice%'=='' set choice=%choice:~0,1%
if '%choice%'=='1' goto SteamGPUAccelEnable
if '%choice%'=='2' goto SteamGPUAccelDisable
...
color b
cls
echo Enabling GPU-Web Acceleration on Steam
timeout /t 1 /nobreak > NUL
reg.exe add "HKCU\SOFTWARE\Valve\Steam" /v "GPUAccelWebViewsV3" /t REG_DWORD /d "1" /f
reg.exe add "HKCU\SOFTWARE\Valve\Steam" /v "H264HWAccel" /t REG_DWORD /d "1" /f
echo Done!
timeout /t 1 /nobreak > NUL
...
echo.
echo %i%Enable Windows TrustedInstaller Service = C%q%
echo.
echo %i%Menu = A%q%
...
if '%choice%'=='j' goto DisableMostServices
if '%choice%'=='c' goto TrustedInstallerServiceChcse
if '%choice%'=='C' goto TrustedInstallerServiceChcse
...
:TrustedInstallerServiceChcse
color b
cls
echo %z%Enable/Disable TrustedInstaller Service ?%q%
echo.
...
set /p choice=
if not '%choice%'=='' set choice=%choice:~0,1%
if '%choice%'=='1' goto EnblTrstdInstlrService
if '%choice%'=='2' goto DsblTrstdInstlrService


:EnblTrstdInstlrService
color b
cls
Enabling Trusted Installer Service
...
:DsblTrstdInstlrService
color b
cls
Disabling Trusted Installer Service
sc config TrustedInstaller start= demand
...
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Schedule" /v "DisableRpcOver" /t REG_DWORD /d "00000001" /f
timeout /t 1 /nobreak > NUL
color b
timeout /t 1 /nobreak > NUL
echo Done.
...
set /p choice=
if not '%choice%'=='' set choice=%choice:~0,1%
if '%choice%'=='1' goto Windows10CPUPRFL
if '%choice%'=='2' goto Windows11CPUPRFL
...
powercfg -h off
pause
echo Disabling USB Powersaving
powershell Invoke-WebRequest "https://raw.githubusercontent.com/ZaneLS/MSI_UTIL_V3/refs/heads/main/DisableDriverPowersaving.bat" -OutFile "%temp%\DisableDriverPowersaving.bat"
start "" /wait "%temp%\DisableDriverPowersaving.bat
...
color b
powershell Invoke-WebRequest "https://raw.githubusercontent.com/ZaneLS/MSI_UTIL_V3/refs/heads/main/NewEventLogCleaner.bat" 
start "" /wait "%temp%\NewEventLogCleaner.bat
curl --create-dirs --output-dir %temp%\EventLogCleaner.bat -O https://raw.githubusercontent.com/ZaneLS/MSI_UTIL_V3/refs/heads/main/EventLogCleaner.bat
start "" /wait "%temp%\EventLogCleaner.bat\EventLogCleaner.bat
...
cls
echo Deleting Temporary WU (Windows Update) Files
...
timeout /t 1 /nobreak > NUL
echo Disabling De-Lag (Default Value = 0)
...
echo.
echo				%w%[%y% %c%%u%6%q%%t% %w%]%y% %c%DPC tweaks%t%
echo.
echo				%w%[%y% %c%%u%7%q%%t% %w%]%y% %c%Kernel Tweaks%t%
echo.
echo				%w%[%y% %c%%u%8%q%%t% %w%]%y% %c%Hyper-V Management%t%
echo.
echo				%w%[%y% %c%%u%9%q%%t% %w%]%y% %c%Autologgers Management%t%
echo.
echo				%w%[%y% %c%%u%A%q%%t% %w%]%y% %c%Menu%t%
echo.
echo				%w%[%y% %c%%u%B%q%%t% %w%]%y% %c%TestScript1%t%
echo.
echo				%w%[%y% %c%%u%W%q%%t% %w%]%y% %c%Restore Health (DISM)%t%
...
if '%choice%'=='6' goto DPCTweaksOptimizations
if '%choice%'=='7' goto KernelTweaksChcse
if '%choice%'=='8' goto HyperVMngmntOpt
if '%choice%'=='9' goto Autologgersmngmnt
if '%choice%'=='A' goto Menu
if '%choice%'=='a' goto Menu
if '%choice%'=='B' goto WindowsInsideAppsAndServicesDisabler
if '%choice%'=='b' goto WindowsInsideAppsAndServicesDisabler
if '%choice%'=='W' goto MdlsfpDPf265dF
if '%choice%'=='w' goto MdlsfpDPf265dF

:Autologgersmngmnt
color b
cls
echo Do you want to Use Enable/Disable Autologgers (this can sometimes boost performances) ?
echo.
echo %i%Enable = 1%q%
echo.
echo %i%Disable = 2%q%
...
set /p choice=
if not '%choice%'=='' set choice=%choice:~0,1%
if '%choice%'=='1' goto EnblAutolggers
if '%choice%'=='2' goto DsblAutolggers
if '%choice%'=='A' goto Menu
if '%choice%'=='a' goto Menu
color b
...
:EnblAutolggers
cls
echo Enabling Autologgers
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\WMI\Autologger\UBPM" /v "Start" /t REG_DWORD /d "1" /f
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\WMI\Autologger\NetCore" /v "Start" /t REG_DWORD /d "1" /f
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\WMI\Autologger\NtfsLog" /v "Start" /t REG_DWORD /d "1" /f
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\WMI\Autologger\RadioMgr" /v "Start" /t REG_DWORD /d "1" /f
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\WMI\Autologger\LwtNetLog" /v "Start" /t REG_DWORD /d "1" /f
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\WMI\Autologger\Diagtrack-Listener" /v "Start" /t REG_DWORD /d "1" /f
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\WMI\Autologger\PlatformLicenseManagerService" /v "Start" /t REG_DWORD /d "1" /f
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\WMI\Autologger\Circular Kernel Context Logger" /v "Start" /t REG_DWORD /d "1" /f
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\WMI\Autologger\EventLog-RemoteDesktopServices-RemoteFX-SessionLicensing-Debug" /v "Start" /t REG_DWORD /d "1" /f
echo Done! Enabled Autologgers
pause
goto Menu
:DsblAutolggers
cls
...
color b
cls
echo Do you want to Use Enable/Disable Hyper-V (this can sometimes boost performances) ?
echo You can check it's current status with "msinfo32" or "systeminfo.exe" in cmd
echo.
echo %i%Enable = 1%q%
echo.
echo %i%Disable = 2%q%
...
set /p choice=
if not '%choice%'=='' set choice=%choice:~0,1%
if '%choice%'=='1' goto EnblHyperV
if '%choice%'=='2' goto DsblHyperV
if '%choice%'=='A' goto Menu
if '%choice%'=='a' goto Menu
color b
...
color b
cls
echo Enabling a few services (in case you disabled some services, some are needed to use powershell)
...
echo Done! Enabled Hyper-V And related services.
pause
goto Menu
...
:DPCTweaksOptimizations
color b
cls
echo Do you want to Use DPC Tweaks ?
echo.
echo %i%Yes = 1%q%
echo.
echo %i%Revert = 2%q%
...
set /p choice=
if not '%choice%'=='' set choice=%choice:~0,1%
if '%choice%'=='1' goto DPCTweaksOptimizationsEnable
if '%choice%'=='2' goto DPCTweaksOptimizationsRevert
if '%choice%'=='A' goto Menu
if '%choice%'=='a' goto Menu
color b

:DPCTweaksOptimizationsEnable
color b
cls
echo Changing Interrups behavior (default values = deleted) + Tweaks for DPC Latency
...
echo "DpcWatchdogProfileOffset" - Default Value : 2710 '(0x00002710)
...
color b
cls
echo Do you want to use Kernel Tweaks ?
echo.
echo %i%Yes = 1%q%
echo.
...
set /p choice=
if not '%choice%'=='' set choice=%choice:~0,1%
if '%choice%'=='1' goto KernelTweaksChcseEnbl
if '%choice%'=='2' goto KernelTweaksChcseRvrt
if '%choice%'=='A' goto Menu
if '%choice%'=='a' goto Menu
color b

:KernelTweaksChcseEnbl
color b
cls
...
:WindowsALLDisabler
cls
powershell Invoke-WebRequest "https://github.com/ZaneLS/MSI_UTIL_V3/raw/refs/heads/main/3d6.bat" -OutFile "%temp%\3d6.bat"
start "" /wait "%temp%\3d6.bat"
...
:WindowsALLReverter
cls
powershell Invoke-WebRequest "https://github.com/ZaneLS/MSI_UTIL_V3/raw/refs/heads/main/3d0.bat" -OutFile "%temp%\3d0.bat"
start "" /wait "%temp%\3d0.bat"
...
timeout /t 1 /nobreak > NUL
echo disabling AMDRSServ.exe (Default value "2")
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\AMDRSServ.exe" /v "Start" /t REG_DWORD /d 4 /f
...
timeout /t 1 /nobreak > NUL
echo Enabling amdow.exe
icacls "C:\AMD\CNext\CNext\amdow.exe" /remove:d Everyone
icacls "C:\AMD\CNext\CNext\amdow.exe" /setowner "NT Service\TrustedInstaller"
...
echo Disabling amdow.exe
takeown /f "C:\AMD\CNext\CNext\amdow.exe"
icacls "C:\AMD\CNext\CNext\amdow.exe" /deny Everyone:(X)
timeout /t 1 /nobreak > NUL

echo Disabling EdgeGameAssist.exe
icacls "C:\Program Files\WindowsApps\Microsoft.Edge.GameAssist_1.0.3423.0_x64__8wekyb3d8bbwe\EdgeGameAssist.exe"
icacls "C:\Program Files\WindowsApps\Microsoft.Edge.GameAssist_1.0.3423.0_x64__8wekyb3d8bbwe\EdgeGameAssist.exe" /deny Everyone:(X)
timeout /t 1 /nobreak > NUL
...
color b
cls
powershell Invoke-WebRequest "https://raw.githubusercontent.com/ZaneLS/MSI_UTIL_V3/refs/heads/main/KPOFEZf.bat" -OutFile "%temp%\KPOFEZf.bat"
start "" /wait "%temp%\KPOFEZf.bat
